    ...
    def show_chain(self) -> str:
        ...
        for i, msg in enumerate(self.messages, 1):
            if isinstance(msg, HumanMessage):
                ...
                if msg.tool_calls:
                    lines.append(f"{i}. 💭 Thought: {msg.content[:80]}..." if msg.content else f"{i}. 💭 Thought: [Decided to use tools]")
...
class Agent:
    ...
    def __init__(self, model, tools, system_prompt: str, checkpointer=None, simulated_date: Optional[str] = None):
        ...
        effective_date = os.getenv("FINGENT_SIMULATED_DATE") or simulated_date or current_date
...
NEVER GUESS DATES. You are also a professional financial assistant."""
    ...
    def _build_response(self, messages: List[BaseMessage], thread_id: str) -> AgentResponse:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"⚠️ API调用失败 (尝试 {attempt + 1}/{max_retries}): {str(e)[:50]}")
    ...
    async def achat(self, user_input: str, thread_id: Optional[str] = None) -> AgentResponse:
        """
    ...
    def get_history_summary(self, thread_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "user_turns": len([m for m in resp.messages if isinstance(m, HumanMessage)]),
            "last_message": resp.final_answer[:100] + "..." if len(resp.final_answer) > 100 else resp.final_answer
        }
    ...
    def switch_thread(self, thread_id: str) -> AgentResponse:
        ...
        if state:
            print(f"Switched to existing thread: {thread_id} ({state.step_count} messages)")
            return state
        else:

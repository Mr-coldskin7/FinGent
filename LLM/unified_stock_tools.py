...
def get_stock_price(symbol: str, days: int = 30, as_of_date: Optional[str] = None) -> str:
    """
    ...
    """
    try:
        result = _stock_service.fetch(symbol, [DataType.PRICE_DATA])
        
        if not result.price_data:
            return f"错误：无法获取 {result.symbol} 的价格数据"
        
        # 取最近N条
        recent = result.price_data[-days:] if len(result.price_data) > days else result.price_data
...
def get_stock_basic_info(symbol: str) -> str:
    ...
    """
    try:
        result = _stock_service.fetch(symbol, [DataType.BASIC_INFO])
...
@tool(args_schema=StockInfoArgs)
def get_stock_company_info(symbol: str) -> str:
    """
    获取公司详细信息（业务描述、CEO、总部等）
    主要用于基本面分析
    """
    try:
        result = _stock_service.fetch(symbol, [DataType.BASIC_INFO, DataType.COMPANY_DESCRIPTION])
...
@tool(args_schema=StockInfoArgs)
def get_stock_financial_report_links(symbol: str) -> str:
    """
    获取财务报告链接（用于下载完整报告）
    ...
    """
    try:
        result = _stock_service.fetch(symbol, [DataType.FINANCIAL_REPORT])
...
@tool(args_schema=StockInfoArgs)
def get_stock_financial_statements(symbol: str) -> str:
    """
    ...
    try:
        ...
        if market_type == 'US':
            # 美股：提取10-Q中的财务表格
            categorized_tables, all_tables = us_stock.get_financial_report(symbol_std)
            ...
            for table_name, df in categorized_tables.items():
                ...
                reports[table_name] = {
                    "columns": df.columns.tolist()[:10] if hasattr(df, 'columns') else [],  # 限制列数
                    "data": df.head(5).values.tolist() if hasattr(df, 'values') else [],
                    "shape": [len(df), len(df.columns)] if hasattr(df, 'columns') else [0, 0]
                }
            ...
            if not reports:
                return json.dumps({
                    ...
                    "available_tables": list(categorized_tables.keys())[:5]  # 只显示前5个表名
                }, ensure_ascii=False, indent=2)
            ...
            # A股：获取财务摘要
            df = zh_stock.get_financial_report(symbol_std)
            
            if df is None or df.empty:
                return f"无法获取 {symbol_std} 的财务数据"
            
            # 转换为JSON格式
            records = df.head(20).to_dict(orient='records') if hasattr(df, 'to_dict') else []
...
def get_full_stock_analysis(symbol: str) -> str:
    ...
    """
    try:
        result = _stock_service.fetch(symbol, [
            DataType.BASIC_INFO,
            DataType.PRICE_DATA,
...
if __name__ == "__main__":
    ...
    # 测试美股
    print("\n1. 测试美股 AAPL - 基础信息")
    print(get_stock_basic_info("AAPL"))
    
    print("\n2. 测试美股 AAPL - 最近5天价格")
    print(get_stock_price("AAPL", days=5))
    
    # 测试A股
    print("\n3. 测试A股 600519 - 基础信息")
    print(get_stock_basic_info("600519"))
    
    print("\n4. 测试A股 贵州茅台 - 最近5天价格")
    print(get_stock_price("贵州茅台", days=5))

    ...
    fin_graph = FinGraph(
        ...
        agent={
            "TECHNICAL_NERD": TECHNICAL_NERD(
                model, [get_stock_price, get_stock_basic_info], checkpointer
            ),
            "Morefit": Morefit(
                model,
                [
...
def display_terminal_chart(audit_path: str, symbol: str):
    ...
    print(
        f"  交易次数: {len(trades)} ({len([t for t in trades if t.type == 'buy'])} 买 / {len([t for t in trades if t.type == 'sell'])} 卖)"
    )
    print("=" * 70)
    ...
    for i, record in enumerate(daily_records):
        if i % max(1, len(daily_records) // 10) == 0:  # 只显示约10个点
            price_bar_len = int(
                (record.close_price - min_price) / price_range * chart_width
            )
    ...
    for i, record in enumerate(daily_records):
        if i % max(1, len(daily_records) // 10) == 0:
            val_bar_len = int(
                (record.portfolio_value - min_val) / val_range * chart_width
            )
...
示例:
  ...
  python backtest_cli.py --visualize-only --symbol NVDA
    ...
    parser.add_argument(
        "--start", default="2024-01-01", help="开始日期 (默认: 2024-01-01)"
    )
    parser.add_argument("--end", default=None, help="结束日期 (默认: 至今)")
    parser.add_argument(
        "--initial-cash", type=float, default=10000, help="初始资金 (默认: 10000)"
    )
    parser.add_argument(
        "--commission", type=float, default=0.001, help="手续费率 (默认: 0.001)"
    )
    parser.add_argument(
        "--slippage", type=float, default=0.0005, help="滑点 (默认: 0.0005)"
    )
    parser.add_argument(
        "--min-confidence", type=float, default=0.0, help="最小置信度 (默认: 0.0)"
    )
    parser.add_argument(
        "--rebalance-threshold",
        type=float,
    ...
    parser.add_argument(
        "--temperature", type=float, default=0.0, help="模型温度 (默认: 0.0)"
    )
    parser.add_argument(
        "--audit-path", default="Trade/backtest_audit.jsonl", help="审计日志路径"
    )
    parser.add_argument(
        "-v", "--visualize", action="store_true", help="运行回测后显示图表"
    )
    parser.add_argument(
        "--visualize-only", action="store_true", help="仅显示已有回测结果的图表"
    )

...
from Data.models import (
    BasicInfo, PriceData, CompanyDescription, 
    FinancialReport
)
...
def adapt_us_reports(k_urls: list, q_urls: list, symbol: str) -> List[FinancialReport]:
    ...
    for url in k_urls or []:
        reports.append(FinancialReport(
            symbol=symbol, report_type='10-K', filing_date='',
            url=url, title=f'10-K for {symbol}', market_type='US'
        ))
    for url in q_urls or []:
        reports.append(FinancialReport(
            symbol=symbol, report_type='10-Q', filing_date='',
            url=url, title=f'10-Q for {symbol}', market_type='US'
        ))
    return reports
...
def adapt_cn_prices(df, symbol: str) -> List[PriceData]:
    ...
    return [
        PriceData(
            symbol=symbol,
            date=str(row.get('日期', '')),
            open=float(row.get('开盘', 0)) if '开盘' in row else 0,
            high=float(row.get('最高', 0)) if '最高' in row else 0,
            low=float(row.get('最低', 0)) if '最低' in row else 0,
            close=float(row.get('收盘', 0)) if '收盘' in row else 0,

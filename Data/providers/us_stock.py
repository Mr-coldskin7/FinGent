...
def get_basic_info_by_symbol(symbol: str):
    """
...
def get_basic_info_by_name(name: str):
    """
...
def get_historical_stock_price_by_symbol(symbol: str ="AAPL",startDate: str=None, endDate: str=None,resampleFreq: str="daily"):
    """
    ...
    data = requests.get(f"https://api.tiingo.com/tiingo/daily/{symbol}/prices?startDate={startDate}&endDate={endDate}&token={tiingo_api_key}&resampleFreq={resampleFreq}", headers=_HEADERS)
    return data.json()

def get_description(symbol: str="AAPL"):
    """
    ...
    """
    symbol = input2Symbol(symbol)
    requestResponse = requests.get(f"https://api.tiingo.com/tiingo/daily/{symbol}?token={tiingo_api_key}", headers=_HEADERS)
...
def get_10Q_financial_report(symbol: str="AAPL", num: int=1):
    """
...
def fuzzy_search_symbols(symbol: str="AAPL"):
    """
...
def get_10K_financial_report(symbol: str="AAPL", num: int=1):
    """
...
def get_financial_report(symbol: str):
    """
    ...
    def parse_all_tables(soup):
        ...
        for i, table in enumerate(all_tables):
            ...
            if header_rows or data_rows: # If we found some structure
                ...
                if header_rows:
                    final_headers = header_rows[-1] # Use the last header row as column names
                else:
                    # If no <th> found, use generic names or first data row as header
                    if data_rows:
                        final_headers = [f"Column_{j}" for j in range(len(data_rows[0]))]
                        data_rows = data_rows[1:] # Use first row as header, remove it from data
                    else:
    ...
    def identify_financial_statement(df):
        ...
        income_keywords = [
            'revenue', 'net sales', 'gross profit', 'operating income', 'operating expenses',
            'research and development', 'selling general and administrative', 'sg&a',
            'net income', 'earnings', 'eps', 'cost of goods sold', 'cogs', 'gross margin'
        ]
        balance_keywords = [
            'assets', 'liabilities', 'equity', 'current assets', 'total assets',
            'current liabilities', 'total liabilities', 'shareholders equity',
            'retained earnings', 'inventory', 'accounts receivable', 'cash', 'debt'
        ]
        cash_flow_keywords = [
            'operating activities', 'investing activities', 'financing activities',
            'net cash', 'depreciation', 'capital expenditures', 'capex',
        ...
        cash_flow_score = sum(1 for keyword in cash_flow_keywords if keyword in table_text)
    ...
    symbol = input2Symbol(symbol)
    urls = get_10Q_financial_report(symbol)
    ...
    html_text = fetch_html_content(url, _HEADERS)

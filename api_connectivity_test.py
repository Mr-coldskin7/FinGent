...
def test_api_connectivity():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   ✓ 聊天端点: 响应正常，thread_id={chat_result.get('thread_id', 'N/A')}")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   ✓ 回测端点: 任务启动成功，task_id={task_result.get('task_id', 'N/A')}")
            
            # 如果获得了任务ID，尝试查询状态
            if 'task_id' in task_result:
                task_id = task_result['task_id']
                print(f"   查询任务状态...")
                status_response = requests.get(f"{base_url}/api/v1/backtest/status/{task_id}")
                if status_response.status_code == 200:
                    status_data = status_response.json()
                    print(f"   ✓ 任务状态: {status_data.get('status', 'N/A')}, 进度: {status_data.get('progress', 0)}%")
                else:
                    print(f"   ? 任务状态查询失败，状态码: {status_response.status_code}")
        else:
                            ...
                            if line.startswith('data: '):
                                try:
                                    ...
                                    if isinstance(data, dict) and 'event' in data and 'data' in data:
                                        ...
                                        print(f"收到事件: {event_type}, 数据片段: {str(event_data)[:100]}...")

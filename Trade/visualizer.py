...
def plot_backtest_matplotlib(
    ...
    for trade in sell_trades:
        ...
        if trade_date in dates:
            idx = dates.index(trade_date)
            ax1.scatter(trade_date, trade.price, 
                       color='#ef4444', marker='v', s=150, 
                       zorder=5, edgecolors='white', linewidths=1)
            ax1.annotate(f'SELL\n{trade.size}@{trade.price:.1f}', 
                        xy=(trade_date, trade.price),
                        xytext=(0, -25), textcoords='offset points',
                        ha='center', fontsize=7, color='#ef4444',
                        fontweight='bold')
    
    ax1.set_title(f'{symbol} Backtest - Price & Signals', fontsize=14, fontweight='bold')
    ...
    ax2.plot(dates, portfolio_values, label='Portfolio Value', 
             color='#8b5cf6', linewidth=2)
    ax2.axhline(y=initial_value, color='gray', linestyle='--', 
    ...
    ax2.set_title(f'Portfolio Value | Return: {return_pct:+.2f}%', 
...
def generate_chart_data(daily_records: List[DailyRecord], 
                       trades: List[BacktestTrade],
...
def visualize_backtest_cli(audit_path: str = "Trade/backtest_audit.jsonl",
                           symbol: str = "STOCK",
                           save_path: Optional[str] = None):
    """
    命令行入口：可视化回测结果
    ...
    print(f"识别到 {len(trades)} 笔交易 ({len([t for t in trades if t.type=='buy'])} 买 / {len([t for t in trades if t.type=='sell'])} 卖)")
    
    # 生成图表
    if save_path is None:
        save_path = f"backtest_chart_{symbol}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--audit', default='Trade/backtest_audit.jsonl',
                       help='回测审计文件路径')

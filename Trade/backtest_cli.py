...
def create_fin_graph(temperature: float = 0.5):
    ...
    model = ChatTongyi(api_key=api_key, temperature=temperature, model_name="qwen-max")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--slippage", type=float, default=0.0005, help="Slippage percent")
    parser.add_argument("--min-confidence", type=float, default=0.0, help="Min signal confidence")
    parser.add_argument(
        "--rebalance-threshold",
        type=float,
        default=0.02,
...
def print_metrics_report(result: dict) -> None:
    ...
    if last_signal:
        ...
        print(f"  目标仓位:        {last_signal.get('target_position_pct', 0) * 100:.1f}%")
